    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                setError(null);
                const response = await import('../data/shakes.json');
                setRecipes(response.default || []);
            } catch (err) {
                console.error('Failed to load shakes data:', err);
                setError('Could not load shakes recipes.');
                setRecipes([]);
            } finally {
                setLoading(false);
            }
    ...
    return (
        <div className="bg-gray-100 min-h-screen py-12">
            <div className="container mx-auto px-4">
                <h1 className="text-4xl font-bold text-orange-800 text-center mb-8">Shakes Recipes</h1>
                {recipes.length === 0 ? (
