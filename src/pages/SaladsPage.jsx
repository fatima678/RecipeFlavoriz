    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                setError(null);
                const response = await import('../data/salad.json');
                setRecipes(response.default || []);
            } catch (err) {
                console.error('Failed to load salad data:', err);
                setError('Could not load salad recipes.');
                setRecipes([]);
            } finally {
                setLoading(false);
            }
    ...
    return (
        <div className="bg-gray-100 min-h-screen py-12">
            <div className="container mx-auto px-4">
                <h1 className="text-4xl font-bold text-orange-800 text-center mb-8">Salad Recipes</h1>
                {recipes.length === 0 ? (
